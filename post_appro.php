...
session_start() ; 
   ...
   // $produitID = $_POST['produitID'] ; 
    ...
    $frns = $db->prepare("SELECT * FROM fournisseur WHERE idFrns = :id") ; 
    $frns->execute([
        'id' => $idFrns ,
    ]);
    ...
    $selectFromAppro = $db->query("SELECT * FROM Approvisionnement") ; 
    $selectFromAppro = $selectFromAppro->fetchAll() ;
    
    //insertion du commande dans la table commande ; 
    if(empty($selectFromAppro)){
        if(isset($idFrns)){

            $appro = $db->prepare('INSERT INTO approvisionnement(dateAchat,idFrns_Frns) VALUES(:dateAchat,:frns)  ') ; 
            $appro->execute([
                    'dateAchat' => $date,
                    'frns' => $idFrns,
...
<body>
    ...
    <div class="content">
        <br>
            <div class="row">
            <form action="panier_frns.php" method = post>
                <input type="hidden" name="idPdr" value="<? echo $prod['idPdr'] ?>">
                <input type="hidden" name="idFrns" value="<? echo $frns['idFrns'] ?>">
                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body">
                        ...
                        <p class="card-text">Prix : <?php echo $prod['prixAchat'] ?> DHs</p>
                        <label for="qteVente" class="form-label">Quantite : </label>
                        <input type="number" class="form-control" name="qteAchat"  required >
                        <br>
                        <a href="panier_frns.php"><button type="submit" name="panier" class="btn btn-outline-success"> <i class='fas fa-cart-plus'></i> Valider l'achat !</button></a>
                    </div>
            ...
            <br><br><br>
            <a href="choice_frns.php"><button type="submit" name="retour" class="btn btn-outline-danger"> <i class='fas fa-prescription-bottle-alt'></i> Supprimer l'achat</button></a>
